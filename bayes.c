...
/* XXX don't actually need to take the root here */
static double termprob_radiusb(struct termprob *tp) {
    double x = tp->p_spam - tp->p_real;
    double y = tp->p_present;
...
static void problist_dump(skiplist s) {
    ...
    for (x = skiplist_itr_first(s), n = 0; x && n < 15;
            x =skiplist_itr_next(s, x), ++n) {
        struct termprob *tp;
...
static void problist_dump2(skiplist s) {
    skiplist_iterator x;
    int n;
    ...
    for (x = skiplist_itr_first(s), n = 0; x && n < 15;
        ...
        tp = skiplist_itr_key(s, x, 0);
        printf("%.*s => %f, %f, %f => %f\n", (int)tp->tlen, tp->term,
                tp->p_real, tp->p_spam, tp->p_present, termprob_radiusb(tp));
    }
}
...
static int termprob_compare2(const void *k1, const size_t k1len,
        const void *k2, const size_t k2len) {
    struct termprob *t1, *t2;
    double r1, r2;
    ...
    r1 = termprob_radiusb(t1);
    r2 = termprob_radiusb(t2);
...
//token_list_dump(tokens);
    ...
    for (si = skiplist_itr_first(tokens); si;
        ...
        if (t.p_spam < 0.01 || t.p_spam > 0.99) continue;
...
double bayes2(skiplist tokens) {
    ...
    skiplist_iterator si;
    size_t nterms, n, nsig = SIGNIFICANT_TERMS;
    double p_spam = 1., p_real = 1.;

//token_list_dump(tokens);
    
    problist = skiplist_new(termprob_compare2);
    db_get_pair("__emails__", &inspamtotal, &inrealtotal);
    if (inspamtotal == 0 || inrealtotal == 0)
        return 0.;
    nspamtotal = inspamtotal; nrealtotal = inrealtotal;
    ...
    for (si = skiplist_itr_first(tokens); si;
        ...
        if (db_get_pair(t.term, &inspam, &inreal)) {
            double nspam = inspam, nreal = inreal;
            t.p_spam = nspam / nspamtotal;
            t.p_real = nreal / nrealtotal;
            t.p_present = (nspam + nreal) / (nspamtotal + nrealtotal);
        }

        if (t.p_spam < 0.01) t.p_spam = 0.01;
        if (t.p_real < 0.01) t.p_real = 0.01;
        //if (t.p_spam > 0.99) t.p_spam = 0.99;

        skiplist_insert(problist, &t, sizeof t, NULL); /* shouldn't fail */
    }

//problist_dump2(problist);
    nterms = skiplist_size(problist);
    if (nsig > nterms) nsig = nterms;
...
//printf("%.*s => %f, %f => %f\n", (int)tp->tlen, tp->term, tp->p_spam, tp->p_present, termprob_radius(tp));
    ...
    /* at this point we should multiply by the priors (i.e. p_spam *= nspamtotal / (nspamtotal + nrealtotal), but at least for testing that is exactly 0.5 in both cases */
    skiplist_delete(problist);

//fprintf(stderr, "p(real) = %g, p(spam) = %g\n", p_real, p_spam);
       
    if (p_real > p_spam) return 0.;
    else return 1.;
}
